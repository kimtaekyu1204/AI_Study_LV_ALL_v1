...
개요:
    이 모듈은 AI 자연어처리 수업의 딥러닝/NLP 파트에서 다루는 인코더 디코더 Seq2Seq에 관한
    모든 코드와 설명을 정리한 파일함.

    원본 Jupyter Notebook: 55 인코더 디코더 Seq2Seq.ipynb

============================================
사용 데이터셋: Tatoeba 영어-프랑스어 병렬 코퍼스
============================================
- 데이터셋명: Tatoeba Project - English-French Sentence Pairs
- 출처: ManyThings.org (http://www.manythings.org/anki/)
        다운로드: http://www.manythings.org/anki/fra-eng.zip
- 설명: 영어-프랑스어 병렬 번역 문장 쌍 데이터셋
- 용도: 기계 번역 (Machine Translation) 학습
- 파일명: fra.txt (탭으로 구분된 텍스트)
- 구성: 약 175,000+ 문장 쌍

데이터 구조:
┌───────────────────────────┬───────────────────────────┬────────────────────┐
│ English (source)          │ French (target)           │ Attribution        │
├───────────────────────────┼───────────────────────────┼────────────────────┤
│ Go.                       │ Va !                      │ CC-BY 2.0 (France) │
│ Run!                      │ Cours !                   │ CC-BY 2.0 (France) │
│ Run!                      │ Courez !                  │ CC-BY 2.0 (France) │
│ Fire!                     │ Au feu !                  │ CC-BY 2.0 (France) │
│ Help!                     │ À l'aide !                │ CC-BY 2.0 (France) │
└───────────────────────────┴───────────────────────────┴────────────────────┘
============================================
"""
